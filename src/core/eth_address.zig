...
pub const EthAddress = struct {
    ...
    pub fn fromHex(hex: []const u8) FromHexError!Self {
        ...
        if (hex[idx_start..].len == 40) {
            ...
            std.mem.writeInt(
                u160,
                &res.inner,
                std.fmt.parseInt(u160, hex, 0) catch
...
test "EthAddress: generate address from hex as string" {
    for (test_addresses) |add| {
        try expectEqual(
            EthAddress.fromInt(try std.fmt.parseInt(u160, add, 0)),
            try EthAddress.fromHex(add),
        );
    }
}
...
test "EthAddress: generate address from hex as string should return an error if hex len is not correct" {
    inline for (test_addresses) |add| {
        try expectError(
            ...
            EthAddress.fromHex(add ++ "32"),
        );
        try expectError(
            FromHexError.UnexpectedLength,
            EthAddress.fromHex("32" ++ add),
        );
        try expectError(
            FromHexError.UnexpectedLength,
            EthAddress.fromHex("32" ++ add ++ "32"),
        );
        try expectError(
            FromHexError.UnexpectedLength,
            EthAddress.fromHex(add[0..5]),
        );
    }
}
    ...
    for (test_addresses) |add| {
        try expectEqual(
            ...
            EthAddress.fromFelt(
                Felt252.fromInt(
                    u256,
                    try std.fmt.parseInt(u160, add, 0),
                ),
            ),
        );
    }
