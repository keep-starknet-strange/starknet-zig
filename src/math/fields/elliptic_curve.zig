...
pub const ECPoint = struct {
    ...
    pub fn ecDouble(self: *Self, alpha: Felt252) ECError!ECPoint {
        ...
        const x = m.powToBigInt(2).sub(&self.x.mul(&Felt252.two()));
        const y = m.mul(&self.x.sub(&x)).sub(&self.y);
        return .{ .x = x, .y = y };
    }
...
test "Elliptic curve math: EC double for valid point A" {
    ...
    const expected_x = Felt252.fromInt(u256, 58460926014232092148191979591712815229424797874927791614218178721848875644);
    const expected_y = Felt252.fromInt(u256, 1065613861227134732854284722490492186040898336012372352512913425790457998694);
...
test "Elliptic curve math: EC double for valid point B" {
    ...
    var to_double = ECPoint{ .x = x, .y = y };
    const actual_ec_point = try to_double.ecDouble(alpha);
    const expected_x = Felt252.fromInt(u256, 1937407885261715145522756206040455121546447384489085099828343908348117672673);
    const expected_y = Felt252.fromInt(u256, 2010355627224183802477187221870580930152258042445852905639855522404179702985);
    const expected_ec_point = ECPoint{ .x = expected_x, .y = expected_y };
    try expectEqual(expected_ec_point, actual_ec_point);
}

test "Elliptic curve math: EC double for valid point C" {
    const x = Felt252.fromInt(u256, 634630432210960355305430036410971013200846091773294855689580772209984122075);
    const y = Felt252.fromInt(u256, 904896178444785983993402854911777165629036333948799414977736331868834995209);
    const alpha = Felt252.one();
    var to_double = ECPoint{ .x = x, .y = y };
    const actual_ec_point = try to_double.ecDouble(alpha);
    const expected_x = Felt252.fromInt(u256, 3143372541908290873737380228370996772020829254218248561772745122290262847573);
    const expected_y = Felt252.fromInt(u256, 1721586982687138486000069852568887984211460575851774005637537867145702861131);
    const expected_ec_point = ECPoint{ .x = expected_x, .y = expected_y };
    try expectEqual(expected_ec_point, actual_ec_point);
}

test "Elliptic curve math: EC add for valid pair of points A and B" {
    const x_a = Felt252.fromInt(u256, 1183418161532233795704555250127335895546712857142554564893196731153957537489);
    const y_a = Felt252.fromInt(u256, 1938007580204102038458825306058547644691739966277761828724036384003180924526);
    const x_b = Felt252.fromInt(u256, 1977703130303461992863803129734853218488251484396280000763960303272760326570);
    const y_b = Felt252.fromInt(u256, 2565191853811572867032277464238286011368568368717965689023024980325333517459);
    var point_a = ECPoint{ .x = x_a, .y = y_a };
    const point_b = ECPoint{ .x = x_b, .y = y_b };
    const actual_ec_point = try point_a.ecAdd(point_b);
    const expected_x = Felt252.fromInt(u256, 1977874238339000383330315148209250828062304908491266318460063803060754089297);
    const expected_y = Felt252.fromInt(u256, 2969386888251099938335087541720168257053975603483053253007176033556822156706);
    const expected_ec_point = ECPoint{ .x = expected_x, .y = expected_y };
    try expectEqual(expected_ec_point, actual_ec_point);
}

test "Elliptic curve math: EC add for valid pair of points C and D" {
    const x_c = Felt252.fromInt(u256, 3139037544796708144595053687182055617920475701120786241351436619796497072089);
    const y_c = Felt252.fromInt(u256, 2119589567875935397690285099786081818522144748339117565577200220779667999801);
    const x_d = Felt252.fromInt(u256, 3324833730090626974525872402899302150520188025637965566623476530814354734325);
    const y_d = Felt252.fromInt(u256, 3147007486456030910661996439995670279305852583596209647900952752170983517249);
    var point_c = ECPoint{ .x = x_c, .y = y_c };
    const point_d = ECPoint{ .x = x_d, .y = y_d };
    const actual_ec_point = try point_c.ecAdd(point_d);
    const expected_x = Felt252.fromInt(u256, 1183418161532233795704555250127335895546712857142554564893196731153957537489);
    const expected_y = Felt252.fromInt(u256, 1938007580204102038458825306058547644691739966277761828724036384003180924526);
    const expected_ec_point = ECPoint{ .x = expected_x, .y = expected_y };
    try expectEqual(expected_ec_point, actual_ec_point);
}
...
test "Elliptic curve math: compute_ec_op_impl_valid_a" {
    const partial_sum = ECPoint{
        .x = Felt252.fromInt(u256, 3139037544796708144595053687182055617920475701120786241351436619796497072089),
        .y = Felt252.fromInt(u256, 2119589567875935397690285099786081818522144748339117565577200220779667999801),
    };
    const doubled_point = ECPoint{
        .x = Felt252.fromInt(u256, 874739451078007766457464989774322083649278607533249481151382481072868806602),
...
test "Elliptic curve math: compute_ec_op_impl_valid_b" {
    const partial_sum = ECPoint{
        .x = Felt252.fromInt(u256, 2962412995502985605007699495352191122971573493113767820301112397466445942584),
        .y = Felt252.fromInt(u256, 214950771763870898744428659242275426967582168179217139798831865603966154129),
    };
    const doubled_point = ECPoint{
        .x = Felt252.fromInt(u256, 874739451078007766457464989774322083649278607533249481151382481072868806602),
        .y = Felt252.fromInt(u256, 152666792071518830868575557812948353041420400780739481342941381225525861407),
